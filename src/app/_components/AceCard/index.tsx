...
import { icons, Logo, Icon, Icons } from '@/app/_components/ProjectCard/icons';

type Stack = string[];

export function CardDemo({ stack }: { stack: Stack }) {
  return (
    <>
      {/* <CardSkeletonContainer> */}
      <Skeleton stack={stack} /> {/* </CardSkeletonContainer> */}
...
const Skeleton = ({ stack }: { stack: Stack }) => {
  const scale = [1, 1.1, 1];
      ...
      <div className="flex flex-row flex-shrink-0 justify-center items-center gap-2">
        {stack && stack.length > 0 && (
          <Container className="h-8 w-8 circle-1">
            <Logo item={stack[0] as keyof Icons} className={'h-4 w-4'} />
          </Container>
        )}
        {stack && stack.length > 1 && (
          <Container className="h-12 w-12 circle-2">
            <Logo
              item={stack[1] as keyof Icons}
        ...
        {stack && stack.length > 1 && (
          <Container className="h-12 w-12 circle-4">
            <Logo item={stack[1] as keyof Icons} className="h-6 w-6 " />
          </Container>
        )}
        {stack && stack.length > 1 && (
          <Container className="h-8 w-8 circle-5">
            <Logo item={stack[1] as keyof Icons} className="h-4 w-4 " />
          </Container>
        )}
      </div>
