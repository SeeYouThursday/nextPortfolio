export const currentProjects = [
  // {
  //   id: 0,
  //   title: 'NextWeather',
  //   shortTitle: 'NW',
  //   github: 'https://github.com/SeeYouThursday/nextWeather',
  //   href: 'https://next-weather-lac.vercel.app/',
  //   description:
  //     'The Weather App allows users to get the current weather information for a specific location. It utilizes the OpenWeather API to fetch weather data and Material-Tailwind for the user interface.',
  //   shortDescrip: 'Check the weather before grabbing that jacket!',
  //   src: '/logo/weather-next.png',
  //   icon: '/logo/weather-next.png',
  //   underConstruction: true,
  //   stack: ['Coming Soon!'],
  // },
  {
    id: 1,
    title: 'iBall24/7',
    shortTitle: 'iBall',
    github: 'https://github.com/',
    href: 'www.iball247.com',
    description: 'Ecommerce - Basketball Training Program Merch Site',
    shortDescrip: 'Ecommerce - Basketball Training Program Merch Site',
    src: '',
    icon: '',
    underConstruction: true,
    stack: ['React, Nextjs, Shopify CMS'],
    business: true,
  },
];

const projectList = [
  {
    id: 0,
    title: 'SeeYouThursday Web Dev Design',
    shortTitle: 'SYT',
    github: 'https://github.com/SeeYouThursday/SeeYouThursdayInc',
    href: 'https://www.seeyouthursday.dev',
    description:
      'Delivering excellent Full Stack Web Development from design to development to dreams come true!',
    shortDescrip: 'Web Dev Design Site',
    src: '/projects/sytdev.webp',
    icon: '',
    underConstruction: false,
    stack: ['Nextjs', 'React', 'Prisma', 'TailwindCSS'],
    business: true,
  },
  {
    id: 1,
    title: 'iBall 24/7',
    shortTitle: 'iBall',
    github: 'https://github.com/',
    href: 'https://www.iball247.com',
    description: 'Ecommerce - Basketball Training Program Merch Site',
    shortDescrip: 'Ecommerce - Basketball Training Program Merch Site',
    src: '/projects/iball247.webp',
    icon: '/logo/iball-logo.svg',
    underConstruction: false,
    stack: ['Nextjs', 'React', 'Shopify CMS'],
  },

  // {
  //   id: 2,
  //   title: 'NextWeather',
  //   shortTitle: 'NW',
  //   github: 'https://github.com/SeeYouThursday/next-weather',
  //   href: 'https://next-weather-lac.vercel.app/',
  //   description:
  //     'The Weather App allows users to get the current weather information for a specific location. It utilizes the OpenWeather API to fetch weather data and Material-Tailwind for the user interface.',
  //   shortDescrip: 'Check the weather before grabbing that jacket!',
  //   src: '/projects/next-weather.png',
  //   icon: '/logo/nw-logo.svg',
  //   underConstruction: true,
  //   stack: ['Nextjs', 'React', 'Vercel Postgres', 'TailwindCSS'],
  // },
  {
    id: 10,
    title: 'GitFitNotFat',
    shortTitle: 'GitFit',
    github: 'https://github.com/SeeYouThursday/GitFitNotFat',
    href: 'https://seeyouthursday.github.io/GitFitNotFat/',
    description:
      'Pick a recipe then give calories, then user inputs the activity they want to do to burn off those calories. The app then returns the duration of that activity needed to burn off the calories from the food selected in addition to the calories burned for that activity. We learned how to avoid conflicts with merges while working with other collaborators in an agile work enviroment. Also, some APIs do not update their documents, and trial and error takes over.',
    shortDescrip: 'Cook great meals, while burning off those calories!',
    src: '/projects/GitFitNotFat.png',
    icon: '/logo/GitFitLogoProto.png',
    underConstruction: false,
    stack: ['Javascript, CSS, HTML, Third Party APIs'],
  },
  {
    id: 12,
    title: 'Choose Your Own Adventure',
    shortTitle: 'CYOA',
    github: 'https://github.com/SeeYouThursday/Choose-Your-Own-Adventure',
    href: 'https://createyourownadventure-5cf1f78476c3.herokuapp.com/',
    description:
      'This is a choose your own adventure game. The user is presented with a scenario and two choices. The user then selects one of the two choices and is presented with another scenario and two more choices. This continues until the user reaches the end of the game. The user can then choose to play again or exit the game. This project was a great learning experience. We learned how to work with a team and how to use git to collaborate on a project. We also learned how to use the express framework to create a web application.',
    shortDescrip:
      'Choose Your Own Adventure! Take charge of your virtual life choices!',
    src: '/projects/cyoa.png',
    icon: '/logo/',
    underConstruction: false,
    stack: ['Handlebars (template language), Javascript, mySQL'],
  },
  {
    id: 13,
    title: 'Fight My Dad',
    shortTitle: 'FMD',
    github: 'https://github.com/SeeYouThursday/Fight-My-Dad',
    href: 'https://fmd-jade.vercel.app/',
    description: "Don't hold in those thoughts, let them out here!",
    shortDescrip: "My dad can beat up your dad! Let's find out!",
    src: '/projects/fmd.svg',
    icon: '',
    underConstruction: false,
    stack: ['MERN Stack'],
  },
  // {
  //   id: 14,
  //   title: 'Weather This',
  //   shortTitle: 'WT',
  //   github: 'https://github.com/SeeYouThursday/Weather-This',
  //   href: 'https://reactweatherdash-5d1448c64bce.herokuapp.com/',
  //   description:
  //     'The Weather App allows users to get the current weather information for a specific location. It utilizes the OpenWeather API to fetch weather data and Material-Tailwind for the user interface.',
  //   shortDescrip: 'Check the weather before grabbing that jacket!',
  //   src: '/projects/fullWeatherThis.png',
  //   icon: '/logo/',
  //   underConstruction: true,
  //   stack: ['Coming Soon!'],
  // },
  {
    id: 15,
    title: "Not Your Mom's Tech Blog",
    shortTitle: 'Blog',
    github: 'https://github.com/SeeYouThursday/Not-Your-Moms-Tech-Blog',
    href: 'https://not-your-moms-tech-blog-5e4e2d95a074.herokuapp.com/',
    description: "Don't hold in those thoughts, let them out here!",
    shortDescrip: "Don't hold in those thoughts, let them out here!",
    src: '/projects/tech-blog.png',
    icon: '',
    underConstruction: false,
    stack: ['React, MongoDB, Express, Node'],
  },
];

export default projectList;
